...
export WEB_GAME_CLIENT="$PWD/.codex/skills/develop-web-game/scripts/web_game_playwright_client.js"
export WEB_GAME_ACTIONS="$PWD/.codex/skills/develop-web-game/references/action_payloads.json"
```

This repository keeps the skill under `.codex/skills`; run the commands from the repository root.
...
- `$WEB_GAME_CLIENT` (repository path: `.codex/skills/develop-web-game/scripts/web_game_playwright_client.js`) — Playwright-based action loop with virtual-time stepping, screenshot capture, and console error buffering. You must pass an action burst via `--actions-file`, `--actions-json`, or `--click`.

## References

- `$WEB_GAME_ACTIONS` (repository path: `.codex/skills/develop-web-game/references/action_payloads.json`) — example action payloads (keyboard + mouse, per-frame capture). Use these to build your burst.
